...
final class FriendshipCalculator
{
        ...
        foreach ($events as $event) {
            if ($event['type'] === "calculation") {
                if ($previousMovementEvent['type'] === "sentToTeam") {
                    $value = self::calculateTimeInParty($value, $event['date'], $previousMovementEvent['date']);
                ...
                if ($previousMovementEvent['type'] === "sentToTeam") {
                    $preliminaryValue = self::calculateTimeInParty($value, $event['date'], $previousMovementEvent['date']);
                } elseif ($previousMovementEvent['type'] === "sentToDayCare") {
    ...
    private static function calculateTimeInParty(int $value, CarbonImmutable $from, CarbonImmutable $to): int
    {
