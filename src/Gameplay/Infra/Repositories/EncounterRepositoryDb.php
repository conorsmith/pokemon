...
use ConorSmith\Pokemon\Gameplay\Domain\Battle\EncounterTableEntry;
...
final class EncounterRepositoryDb implements EncounterRepository
{
    ...
    {
        $fixedEncounter = $this->findFixedEncounter($location->id, $number);

        if (is_null($fixedEncounter)) {
            throw new LogicException("Case where fixed encounter doesn't exist is unhandled");
        }

        if ($fixedEncounter->canBattle === false) {
            throw new LogicException("Case where player can't battle pokemon is unhandled");
        }

        return $this->generate(
            new EncounterTableEntry(
                $fixedEncounter->pokedexNumber,
                null,
                1,
                $fixedEncounter->level,
                $fixedEncounter->level,
            ),
            $fixedEncounter->id,
            $fixedEncounter->isShiny,
        );
    }

    private function findFixedEncounter(string $locationId, string $pokedexNumber): ?FixedEncounter
    {
